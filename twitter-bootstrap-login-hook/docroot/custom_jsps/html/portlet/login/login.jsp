...
<c:choose>
	...
	<c:otherwise>
		...
		<form action="<%= loginURL %>" method="post" name="fm" >
			...
			<input name="login" type="text" class="span2" placeholder="<%= LanguageUtil.get(pageContext, "user-name") %>" />
			
			<input name="password" type="password" class="span2" placeholder="<%= LanguageUtil.get(pageContext, "the-user-password") %>" />

		
<%-- 			<div id="<portlet:namespace />passwordCapsLockSpan" class="alert"> --%>
...
<!-- 			</div> -->
			...
			<c:if test="<%= company.isAutoLogin() && !PropsValues.SESSION_DISABLED %>">
				<label class="checkbox">
					<input checked="<%= rememberMe %>" name="rememberMe" type="checkbox" ><%= LanguageUtil.get(pageContext, "remember-me") %></input>
				</label>
			</c:if>
		
			<input class="btn" type="submit" value="<%= LanguageUtil.get(pageContext, "sign-in") %>" />
